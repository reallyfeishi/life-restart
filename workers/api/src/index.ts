...
function getWritingStylePrompt(styleId?: string): string {
  return WRITING_STYLE_PROMPTS[styleId || 'warm_novel'] || WRITING_STYLE_PROMPTS['warm_novel'];
}
...
async function handleBackground(body: any, env: Env, headers: Record<string, string>): Promise<Response> {
  const { world, identity, talents, attributes, model, disableThinking, writingStyle } = body;
  ...
  const prompt = buildBackgroundPrompt(world, identity, talents, attributes, writingStyle);
...
async function handleGenerate(body: any, env: Env, headers: Record<string, string>): Promise<Response> {
  const { age, attributes, talents, events, world, identity, resources, model, disableThinking, decision, writingStyle } = body;
  const apiKey = env.AI_API_KEY || '';
  ...
  const prompt = buildEventPrompt(age, attributes, talents, events || [], world, identity, resources || { money: 0, career: '无业', social: 0 }, decision, writingStyle);
  const config = getAIConfig(model, env);
...
${getWritingStylePrompt(writingStyle)}
...
**注意：每个事件应围绕一个核心展开，但可以同时影响多个属性。比如创业事件可能同时影响智力和家境，修行事件可能同时影响体质和颜值。不要每个事件都写完全一样的属性组合——要有变化。事件的核心应该明确，属性变化要自然跟随事件内容。**
...
**【资源联动——极其重要】当前状态：金钱${resources.money} 职业${resources.career} 社交${resources.social}**
resources字段必须在事件中体现变化，不要每次都写0！具体要求：
1. **金钱变动**：约40-50%的事件应该带来金钱变化（正负均可）。比如：工作赚钱、投资获利/亏损、购物消费、获得遗产、被骗钱财、捡到钱、医疗费用、学费支出等。不要只在家境事件中才变动金钱——智力事件（买书/学费）、体质事件（医疗费/健身）、颜值事件（美容/服装）都可能涉及金钱
2. **职业变动**：当事件涉及升学、就业、升职、跳槽、创业、退休等人生阶段时，必须更新career字段。不相关的事件可以保持空字符串
3. **社交变动**：约30-40%的事件应该带来社交变化。比如：结交新朋友、社交活动、人际关系冲突、团队合作、孤独隐居等
...
3. **叙述视角——极其重要：必须使用第二人称"你"来叙述**——事件以玩家的视角展开，用"你"来描述经历、感受和行动。绝对不要用"他""她"或第三人称来叙述玩家的行为，也不要用"我"
...
11. 在末尾用JSON格式标注变化，参考以下示例：

示例1（普通事件+多属性+金钱变动）：
{"content":"你报名参加了绘画课程，用色彩捕捉生活中的美好瞬间。","attrChanges":{"颜值":1,"智力":1},"resources":{"money":-50,"career":"","social":5},"isDecision":false}

示例2（家境事件+资源联动）：
{"content":"你投资的商铺获得了不错的回报，生活变得更加宽裕。","attrChanges":{"家境":1},"resources":{"money":200,"career":"","social":0},"isDecision":false}

示例3（职业变动）：
{"content":"你凭借出色的表现被提拔为部门主管，开始了新的职业生涯。","attrChanges":{"智力":1,"家境":1},"resources":{"money":100,"career":"部门主管","social":10},"isDecision":false}

选择事件格式（仅在重要节点使用）：
{"content":"事件内容...","attrChanges":{"属性":数值},"resources":{"money":数值,"career":"新职业或空串","social":数值},"isDecision":true,"decision":{"prompt":"你决定：","options":[{"id":"a","text":"选项A","hint":"对xxx有极高要求，成功后可能获得xxx，但若失败则可能xxx"},{"id":"b","text":"选项B","hint":"对xxx有xxx要求，成功后可能获得xxx，但若失败则可能xxx"},{"id":"c","text":"选项C","hint":"对xxx有xxx要求，成功后可能获得xxx，但若失败则可能xxx"}],"allowFreeInput":true}}
