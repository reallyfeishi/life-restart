...
- **体质**：锻炼、练武、劳作、饮食规律、修行、调养 → 体质+1到+2；生病、受伤、挨饿、劳累过度、暴饮暴食 → 体质-1到-2
- **颜值**：贵人指点仪态、精心打扮、美容奇遇、气质蜕变、青春焕发 → 颜值+1到+2；遭遇毁容、风吹日晒、不修边幅、容颜衰老 → 颜值-1到-2
- **家境**：获得遗产、投资成功、贵人资助、经商获利、意外之财 → 家境+1到+2；家道中落、被骗、投资失败、灾祸、挥霍 → 家境-1到-2

**属性数值区间的影响（极其重要）：事件描述必须反映当前属性的实际水平！**
- 属性 0-5（低）：事件应描写"笨拙尝试、屡屡碰壁、力不从心、捉襟见肘"等窘境
- 属性 6-10（中）：事件描写"有所进步、勉强应付、偶有亮眼、起伏不定"
- 属性 11-15（高）：事件描写"游刃有余、出类拔萃、得心应手、令人羡慕"

**强制要求：**
1. attrChanges中至少要有1个属性变化（除非是纯叙事性的过渡事件），且变化方向必须与事件内容逻辑一致——绝不能出现"生病卧床"却"体质+1"的矛盾情况。
2. **属性分布均衡——极其重要：在连续生成的事件中，四个属性（颜值、智力、体质、家境）都应该有机会发生变化。不要只集中在智力和体质上反复触发变化。颜值和家境同样重要，需要给它们同等的事件关注度。**
3. 在attrChanges中如实反映所有属性的变化。
